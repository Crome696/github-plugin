...
- Synchronizing a parent issue's owned child-status section after a complete
  sub-issue publication, with current child evidence, deterministic rendering,
  exact body-only authorization, and safe standalone reruns.
...
| `plan-product` | `product-planner-agent` | One ProductSubIssueDrafts v2 canonical set composed before one ProductPlannerRun v2 review and digest-bound approval; publication and the auxiliary ParentTrackerSynchronization v1 result are displayed without a second Command write. |
...
| [`product-planner-agent`](../../agents/product-planner-agent.md) | Turn one verified parent issue into a prioritized graph, consume the canonical ProductSubIssueDrafts v2 set, hand its exact authorized publication to the publication Skill, and return a separate ParentTrackerSynchronization v1 result afterward. |
...
| [`product-interview-policy.mdc`](../../rules/product-interview-policy.mdc) | Makes `conduct-product-interview` the sole adaptive decision owner and requires deterministic consumers to use a complete ProductInterview v2 without inventing essential choices. |
