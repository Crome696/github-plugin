...
3. Display the Agent's exact version-1 `ProductPlannerRun`, including parent
   identity, issue structure, dependency order, parallel groups, confirmed
   priorities, open decisions, drafts, blockers, authorization, and any
   publication or verification result. Preserve partial or blocked results and
   do not perform a second write from the command.
